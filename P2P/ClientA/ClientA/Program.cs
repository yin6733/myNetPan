...
namespace ClientA
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //client.SendTo(Encoding.UTF8.GetBytes("Send"), new IPEndPoint(IPAddress.Parse("106.14.44.67"), 13579));
            /*Socket client2 = new Socket(AddressFamily.InterNetwork, SocketType.Dgram, ProtocolType.Udp);
            client2.Bind(new IPEndPoint(IPAddress.Parse("0.0.0.0"), new Random(DateTime.Now.Second).Next(10000, 12000) + 2));
            client2.SendTo(Encoding.UTF8.GetBytes("Send"), new IPEndPoint(IPAddress.Parse("106.14.44.67"), 13579));*/
            Console.WriteLine(Left(message, ':'));
            Console.WriteLine(int.Parse(Right(message, ':')));
            client.SendTo(Encoding.UTF8.GetBytes("Hello,ClientB!"), new IPEndPoint(IPAddress.Parse(Left(message, ':')), int.Parse(Right(message, ':'))));
